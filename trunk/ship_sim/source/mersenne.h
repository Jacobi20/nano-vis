...
class RandMT {
	...
	static const uint K =       0x9908B0DFU;        // a magic constant

	// If you want a single generator, consider using a singleton class 
	// instead of trying to make these static.
	uint   state[N+1];  // state vector + 1 extra to not violate ANSI C
	uint   *next;       // next random value is computed from here
	uint   initseed;    //
	int      left;        // can *next++ this many times before reloading

	inline uint hiBit(uint u) { 
		return u & 0x80000000U;    // mask all but highest   bit of u
	}

	inline uint loBit(uint u) { 
		return u & 0x00000001U;    // mask all but lowest    bit of u
	}

	inline uint loBits(uint u) { 
		return u & 0x7FFFFFFFU;   // mask     the highest   bit of u
	}

	inline uint mixBits(uint u, uint v) {
...
public:
	RandMT() ;
	RandMT(uint seed) ;
	inline uint randomMT(void) ; 
	inline float randomMTf(void) ; 
...
inline RandMT::RandMT() {
	seedMT(1U);
}


inline RandMT::RandMT(uint seed) {
	seedMT(seed);
}


inline void RandMT::seedMT(uint seed) {
	//
...
inline uint RandMT::reloadMT(void) {
...
inline uint RandMT::randomMT(void) {
	uint y;
...
inline float RandMT::randomMTf( void )
{
	double v = (double)(randomMT()) / 4294967296.0;
	return (float)v;
}
